...
Dispatcher.prototype.dispatch = function(module) {
  ...
  if(module.repository !== undefined && module.repository.type == 'git' && module.repository.url !== undefined && module.repository.url.length > 0) {
...
Dispatcher.prototype.done = function (data) {
  if(data.result !== undefined && data.result !== null) {
    ...
    if(data.result.output !== undefined && data.result.output !== null) {
      this.redis.hset('output', data.module, data.result.output);
    }

    this.redis.hset('codes', data.module, data.result.code);
    this.redis.hset('times', data.module, new Date().getTime());

    this.redis.srem('failed', data.module);
    this.redis.srem('rfailed', data.module);
    this.redis.srem('inexistent', data.module);
    this.redis.srem('ok', data.module);
    this.redis.srem('nok', data.module);

    switch(data.result.result) {
      case 'ok':
        this.redis.sadd('ok', data.module);
        break;
      case 'nottested':
        this.redis.sadd('inexistent', data.module);
        break;
      case 'nok':
        this.redis.sadd('nok', data.module);
        break;
      case 'timedout':
        console.log('TIMEDOUT! ' + data.module);
        this.redis.sadd('failed', data.module);
        this.redis.hset('times', data.module, new Date().getTime());
        break;
      case 'tarball':
        console.log('TARBALL INVALID! ' + data.module);
        this.redis.sadd('rfailed', data.module);
        this.redis.hset('times', data.module, new Date().getTime());
        break;
    }

    console.log('DONE! ' + data.module + ' with code ' + data.result.result + '(' + data.result.code + ')');
  } else {
    console.log('DONE! ' + data.module + ' STAY AS IS!');
  }
};
