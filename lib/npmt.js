...
Npmt.prototype.dispatch = function(module, cb) {
  ...
  var time = new Date(module.time.modified).getTime();

  collection.find({module: module.name, time: {$lt: time}}, {}, function (err, doc) {
    if(err) return cb(err);

    if(doc.length) return self.save(module, cb);
    
    // nothing return callback
    cb();
  });
};
